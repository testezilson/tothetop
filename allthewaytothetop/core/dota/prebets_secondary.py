...
def get_dota_db_path():
    ...
    """
    env_path = (os.environ.get("DOTA_DB_PATH") or "").strip()
    ...
    # No Railway/Linux não existem paths Windows; priorizar o banco versionado em data/.
    prefer_project = (
        bool((os.environ.get("RAILWAY_ENVIRONMENT") or "").strip())
        or (os.environ.get("DOTA_DB_PREFER_APP_DATA", "") or "").lower()
        in ("1", "true", "yes")
    )
    possible_paths = project_paths + desktop_paths if prefer_project else desktop_paths + project_paths
