...
def _res_sql_to_won(r):
    """
    Converte ``res`` (0/1, Win/Loss, etc.) em True/False. None = indeterminado.
    """
    if r is None:
        return None
    if type(r) is bool:
        return r
    if isinstance(r, np.bool_):
        return bool(r)
    if isinstance(r, (float, np.floating)) and pd.isna(r):
        return None
    if isinstance(r, str):
        s = r.strip().lower()
        if s in (
        ...
        if not s or s in ("nan", "none", "null", "-"):
            return None
    if isinstance(r, (np.integer,)) and not isinstance(r, bool):
        try:
            iv = int(r)
...
def _value_expr_for_team_game(stat: str) -> str:
    """
    Agregado por (gameid, time): uma linha lógica por partida, não 5 por role.
    Usa as mesmas definições de ``fetch_team_recent`` (totais da partida em kills/dragons/…; não ``kills`` de jogador).
    Prefixo ``o.`` — alias na subquery.
    """
    s = (stat or "").lower().replace(" ", "")
    if s == "gamelength":
        return "(MAX(COALESCE(o.gamelength, 0)) / 60.0)"
    if s in FIRST_STATS:
        return f"MAX(COALESCE(o.{s}, 0))"
    if s == "kills":
        return "MAX(COALESCE(o.teamkills,0) + COALESCE(o.opponentkills,0))"
    if s == "deaths":
        return "MAX(COALESCE(o.teamdeaths,0) + COALESCE(o.opponentdeaths,0))"
    if s == "assists":
        return "MAX(COALESCE(o.teamassists,0) + COALESCE(o.opponentassists,0))"
    if s == "barons":
        return "MAX(COALESCE(o.barons,0) + COALESCE(o.opp_barons,0))"
    if s == "towers":
        return "MAX(COALESCE(o.towers,0) + COALESCE(o.opp_towers,0))"
    if s == "dragons":
        return "MAX(COALESCE(o.dragons,0) + COALESCE(o.opp_dragons,0))"
    if s in ("heralds", "firstherald", "first_herald"):
        return "MAX(COALESCE(o.firstherald, 0))"
    if s in ("firstblood", "first_blood", "firstblood"):
        return "MAX(COALESCE(o.teamkills,0) + COALESCE(o.opponentkills,0))"
    return "MAX(COALESCE(o.teamkills,0) + COALESCE(o.opponentkills,0))"
...
def fetch_team_recent_with_opponent(conn, team_name, stat, limit_games=10):
    """
    Últimos N *jogos* (não N linhas de jogador). Agrega com ``GROUP BY gameid, time``.

    Cada item: ``{"value", "opponent", "won", "gameid"}``. ``result`` = ``MAX(o.result)`` nesse grupo;
    ``won`` vem dali. Valor = mesma regra de mercado que ``fetch_team_recent`` (totais, teamkills+…, etc.).
    """
    stat = (stat or "").lower()
    team_col = _get_team_column(conn)
    names = _team_query_names(team_name)
    if not names:
        return []
    ph = ", ".join("?" for _ in names)
    val_ex = _value_expr_for_team_game(stat)
    limit_games = int(limit_games) if limit_games else 10
    ...
    query = f"""
        WITH per_team_game AS (
            ...
            WHERE o.{team_col} COLLATE NOCASE IN ({ph})
              AND o.date IS NOT NULL AND TRIM(CAST(o.date AS TEXT)) != ''
            GROUP BY o.gameid, o.{team_col}
        ),
        opponent_side AS (
            SELECT
                o1.gameid,
                o1.{team_col} AS my_team,
                MAX(o2.{team_col}) AS opponent
            FROM oracle_matches o1
            INNER JOIN oracle_matches o2
                ON o1.gameid = o2.gameid AND o1.{team_col} != o2.{team_col}
            WHERE o1.{team_col} COLLATE NOCASE IN ({ph})
            GROUP BY o1.gameid, o1.{team_col}
        )
        SELECT
            p.gameid,
            p.d AS match_date,
        ...
        FROM per_team_game p
        LEFT JOIN opponent_side opp
            ON opp.gameid = p.gameid AND opp.my_team = p.tname
        ORDER BY datetime(TRIM(p.d)) DESC, p.gameid DESC
        LIMIT ?
    """
    try:
        df = pd.read_sql_query(
            query,
            conn,
            params=(*names, *names, limit_games),
        )
        out = []
        for _, row in df.iterrows():
            ...
            r = row.get("res")
            w = _res_sql_to_won(r)
            if w is None and r is not None and not (isinstance(r, float) and pd.isna(r)):
                w = _res_sql_to_won(str(r).strip())
            gid = row.get("gameid")
            opp = row.get("opponent")
            out.append(
                {
                    "gameid": str(gid) if gid is not None else "",
                    "value": float(v),
                    "opponent": str(opp).strip() if pd.notna(opp) and opp is not None else "—",
                    "won": w,
                    "result_raw": r,
                }
            )
