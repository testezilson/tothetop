# Script PowerShell para build com CLEAN COMPLETO
# Remove pastas build/ e dist/ antes de fazer o build
# Uso: .\build_scripts\build_clean.ps1

Write-Host "=== LoL Oracle ML - Build com Clean Completo ===" -ForegroundColor Green
...
Write-Host "`n🧹 Limpando pastas build/ e dist/..." -ForegroundColor Yellow
...
# Remover arquivos .spec antigos (opcional - comentar se quiser manter)
# Get-ChildItem -Path "." -Filter "*.spec" | Remove-Item -Force

Write-Host "`n📦 Iniciando build..." -ForegroundColor Yellow
