#!/usr/bin/env python3
"""
Testa predict de kills finais em jogos do LoL Live (apenas ligas MAJOR).

Parte 1: Cenários com gold override (5k, 10k, 0) aos 10/15/20 min.

Parte 2: Jogos por faixa de gold real:
  0~1k, 1k~2k, 2k~3k, ..., 9k~10k (positivo = Blue ahead)
  -1k~0, ..., -10k~-9k (negativo = Red ahead)

Para cada faixa × minuto: n, MAE, mean_pred, mean_real.
Predição usa gold real do snapshot (inclui draft, towers, baron, etc).

Requer coluna "league" nos snapshots (build_snapshots.py já grava). Filtra para MAJOR.
"""
import math
import pickle
import sys
...
MODEL_PATH = PROJECT_ROOT / "model_artifacts" / "lol_live_kills_remaining.pkl"
CHECKPOINTS = [10, 15, 20]
MU_CLAMP_MIN = 0
MU_CLAMP_MAX = 60

# Ligas MAJOR: apenas esses jogos entram no teste (consistente com TESTE LOL LIVE GAME)
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Testa predict LoL Live com gold override em todos os jogos")
    ...
    # Apenas ligas MAJOR (requer coluna "league" nos snapshots; build_snapshots grava desde que passou a gravar)
    if "league" in df.columns:
        ...
        print(f"Filtrado para ligas MAJOR: {before} -> {len(df)} linhas")
    else:
        print("Aviso: coluna 'league' não encontrada nos snapshots. Rode build_snapshots.py de novo para gravar league; usando todos os jogos.")

    if len(df) == 0:
        print("Nenhum snapshot nos checkpoints 10/15/20 (ou nenhum jogo MAJOR).")
        sys.exit(1)
    ...
    print("=" * 70)
    print("Teste LoL Live: predict kills finais vs real (gold override) — APENAS MAJOR")
    print("=" * 70)
    print(f"Snapshots (MAJOR): {len(df)} linhas (checkpoints 10/15/20)")
    ...
    print("-" * 70)
    print()
    print("2. Jogos por FAIXA DE GOLD real (predição com gold real vs total real):")
    print("-" * 70)

    # Faixas: 0~1k, 1k~2k, ..., 9k~10k (positivo = Blue ahead) e -1k~0, ..., -10k~-9k (negativo = Red ahead)
    GOLD_BUCKETS = [
        ("<-10k", -999999, -10000),
        ("-10k~-9k", -10000, -9000),
        ("-9k~-8k", -9000, -8000),
