...
def find_latest_csv():
    ...
    Prioridade: pasta do .exe (empacotado) > ``Documents/db2026`` (CSV principal) > ``data/`` do projeto.
    ...
    # 1) Empacotado: pastas do .exe (onde o utilizador deixa o CSV)
    ...
    # 2) Pasta canónica do utilizador: Documents/db2026
    db2026_dir = get_lol_db2026_dir()
    if os.path.isdir(db2026_dir):
        p = _pick_newest(_oracle_csv_candidates_in_dir(db2026_dir))
        if p:
            return p

    # 3) data/ do projeto
    p = _pick_newest(_oracle_csv_candidates_in_dir(get_data_dir()))
    if p:
        return p
...
def create_oracle_db_from_csv(csv_path, db_path):
    ...
    for idx, row in df.iterrows():
        try:
            ...
            # Resultado (1 = vitória, 0 = derrota)
            result = 1 if str(row.get('result', '')).lower() in ['1', 'true', 'win', 'w'] else 0 if pd.notna(row.get('result')) else None
